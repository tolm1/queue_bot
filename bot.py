...
def enter_queue(call):
    ...
    if call.data.startswith('enter_'):
        ...
        prep = 'за'
    subj = call.data.split('_')[2] # ахах, случайно починил баг

    key = types.InlineKeyboardMarkup()
    key.add(types.InlineKeyboardButton('Встать в очередь', callback_data=call.data))
    #key.add(types.InlineKeyboardButton(text = 'Выйти', callback_data='df'))

    if call.data == "df":
        #print(1)
        #bot.answer_callback_query(call.id, 'Ты уже в очереди', False)
        bot.send_message('вы нажали на кнопку')
    ...
    if call.inline_message_id not in queues:  # проверка нахождения очереди в списке текущих очередей
        ...
        # если очередь уже была, работаем с ней
        if str(call.from_user.id) not in queues[call.inline_message_id]:
            # если человека нет в очереди
            ...
            text = 'Очередь {} *{}*\n'.format(prep, subj) # не работает отображение названия очереди при записи
            n = 1
